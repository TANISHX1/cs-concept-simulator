 ...
 */
export function getProvider(opts: AiProviderOptions = {}) {
  ...
  const provider = createOpenAICompatible({
    name: "consim",
    baseURL: opts.baseUrl ?? process.env.AI_BASE_URL ?? DEFAULT_BASE_URL,
    apiKey,
  });

  return {
    provider,
    modelId: opts.model ?? process.env.AI_MODEL ?? DEFAULT_MODEL,
  };
}
